...
export const makePlantuml = async (cheerpjPath = "/app") => {
  ...
  return {
    // renderPng is also available via com.plantuml.api.cheerpj.v1.Png
    // but I am not including it here. See https://github.com/plantuml/plantuml/blob/master/src/com/plantuml/api/cheerpj/v1/Png.java
    renderSvg: async (pumlContent) => {
      const rawResult = await cjCall(
        "com.plantuml.api.cheerpj.v1.Svg",
        "convert",
        "light",
        pumlContent
      );
      return new TextDecoder("utf-16").decode(rawResult.value0).slice(1)
    },
  };
};
